angular.module('myApp')
    .controller('project', function ($scope, $http, $window, $rootScope, $cookieStore, $location, $route, $timeout, $routeParams) {
            ...
            $http({
                ...
                for (var i = 0; i < $scope.models.lists[categorieName][0].length; i++) {
                    if ($scope.models.lists[categorieName][0][i].id == taskId) {
                        $scope.models.lists[categorieName][0][i].sous_tache[0].push({
                            checked: false,
                            description: nameSubTasks,
                            id: subTaskid,
                            val: null
                        });
        ...
        $scope.addUserInProject = function (userId) {
        ...
        $scope.addInSubTask = function (UserId, taskId) {
            ...
            $http({
                ...
                if (success.data.creation == 'false') {
                    $scope.message_subsrib_tasks = {'taskId': taskId, 'message': success.data.message};
                }
                if (success.data.creation == 'true') {
                    Object.keys($scope.models.lists).forEach(function (key) {
                        for (var i = 0; i < $scope.personnelProjects.length; i++) {
                            if ($scope.personnelProjects[i].id == id_params) {
                                ...
                                for (var k = 0; k < $scope.models.lists[key][0].length; k++) {
                                    if ($scope.models.lists[key][0][k].id == taskId) {
                                        for (var j = 0; j < $scope.members_project.length; j++) {
                                            if ($scope.members_project[j].id == UserId) {
                                                $scope.models.lists[key][0][k].users_in_tasks[0].push($scope.members_project[j]);
                                            }
                                        }
                                    }
                                }

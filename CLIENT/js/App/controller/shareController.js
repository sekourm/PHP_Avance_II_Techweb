angular.module('myApp')
    .controller('share', function ($scope, $http, $window, $rootScope, $cookieStore, $location, $route, $timeout,$routeParams) {
        console.log("controller share charge");
        ...
        $http({
            ...
            var test = [];
            var element = {};
            $scope.personnelProjects = data.data[0].projects_by_users;
           // console.log($scope.personnelProjects);

            for (var h = 0; h < $scope.personnelProjects.length; h++) {
                if($scope.personnelProjects[h].projects_by_projects.id == id_params){
                    $scope.essay = $scope.personnelProjects[h].projects_by_projects;
                }
            }

            for (var i = 0; i < $scope.personnelProjects.length; i++) {
                if($scope.personnelProjects[i].projects_by_projects.id == id_params){
                    for(var j = 0; j < $scope.personnelProjects[i].projects_by_projects.categories_by_projects.length;j++){
                        var test2 =  $scope.personnelProjects[i].projects_by_projects.categories_by_projects[j].name;
                        element[test2] = [[],[ $scope.personnelProjects[i].projects_by_projects.categories_by_projects[j].id]];
                        test.push({ [test2]: [], id: $scope.personnelProjects[i].projects_by_projects.categories_by_projects[j].id})
                    }
                }
            }
                ...
                for (var i = 0; i < $scope.personnelProjects.length; i++) {
                    if($scope.personnelProjects[i].projects_by_projects.id == id_params){
                        for(var j = 0; j < $scope.personnelProjects[i].projects_by_projects.categories_by_projects.length;j++){
                            ...
                            for(var k = 0; k< $scope.personnelProjects[i].projects_by_projects.categories_by_projects[j].categories_by_tasks.length;k++){
                                if ($scope.models.lists[key][1][0] == $scope.personnelProjects[i].projects_by_projects.categories_by_projects[j].id) {
                                    var countTache = $scope.personnelProjects[i].projects_by_projects.categories_by_projects[j].categories_by_tasks[k].tasks_by_sub_tasks.length;
                                    if(countTache > 0){
                                        var result = 100/countTache;
                                    }

                                        for(var y = 0; y <$scope.personnelProjects[i].projects_by_projects.categories_by_projects[j].categories_by_tasks[k].users_in_tasks_by_tasks.length;y++)
                                        {
                                            if($scope.personnelProjects[i].projects_by_projects.categories_by_projects[j].categories_by_tasks[k].users_in_tasks_by_tasks[y].users_by_tasks != undefined){
                                                users_in_tasks.push({
                                                    id: $scope.personnelProjects[i].projects_by_projects.categories_by_projects[j].categories_by_tasks[k].users_in_tasks_by_tasks[y].users_by_tasks.id,
                                                    username: $scope.personnelProjects[i].projects_by_projects.categories_by_projects[j].categories_by_tasks[k].users_in_tasks_by_tasks[y].users_by_tasks.username,
                                                    picture: $scope.personnelProjects[i].projects_by_projects.categories_by_projects[j].categories_by_tasks[k].users_in_tasks_by_tasks[y].users_by_tasks.picture
                                                });
                                            }
                                        }

                                    for(var m = 0; m <$scope.personnelProjects[i].projects_by_projects.categories_by_projects[j].categories_by_tasks[k].tasks_by_sub_tasks.length;m++){
                                        tab_sous_tache.push({
                                            id:$scope.personnelProjects[i].projects_by_projects.categories_by_projects[j].categories_by_tasks[k].tasks_by_sub_tasks[m].id,
                                            description:$scope.personnelProjects[i].projects_by_projects.categories_by_projects[j].categories_by_tasks[k].tasks_by_sub_tasks[m].description,
                                            val: result,
                                            checked:$scope.personnelProjects[i].projects_by_projects.categories_by_projects[j].categories_by_tasks[k].tasks_by_sub_tasks[m].checked
                                        });

                                        if($scope.personnelProjects[i].projects_by_projects.categories_by_projects[j].categories_by_tasks[k].tasks_by_sub_tasks[m].checked == true)
                                        {
                                    ...
                                    $scope.models.lists[key][0].push({
                                        label: $scope.personnelProjects[i].projects_by_projects.categories_by_projects[j].categories_by_tasks[k].name,
                                        id: $scope.personnelProjects[i].projects_by_projects.categories_by_projects[j].categories_by_tasks[k].id,
            ...
            for (var i = 0; i < $scope.personnelProjects.length; i++) {
                if ($scope.personnelProjects[i].projects_by_projects.id == id_params)
                {
                    for(var n=0; n<$scope.personnelProjects[i].projects_by_projects.users_by_projects.length;n++)
                    {
                        $scope.members_project.push($scope.personnelProjects[i].projects_by_projects.users_by_projects[n].users_by_projects);
                    }
                }
            }
